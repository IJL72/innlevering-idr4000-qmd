...
Min hypotese er at trening med tre sett vil gi et større respons enn trening med ett sett.
...
Parret t-test sammenligner endringen i fettfri masse gjennom intervensjonen i relasjon til single- eller multi-sett trening Ved parret t-test får vi t = 2.1875, df = 33, p-verdi = 0.0359.
Estimat for gjennomsnittet av forskjellen er 122,79 og 95% konfidensinterval 8,58 og 237,00.
Dette viser at endringen i fettfri masse mellom de to gruppene var signifikant.
...
str.dat <- strengthvolume %>% 
  select(participant, sex, exercise, include, time, sets, leg, load ) %>%
        ...
        # Filtrerer  includeerte deltakere
        filter(include == "incl") %>%
        ...
        #  Velger legpress-test  og bare pre-post data
        filter(exercise == "legpress",
                time %in% c("pre", "post")) %>%
        # Utvider datasettet 
        pivot_wider(names_from = "time",
                    values_from = "load") %>%
        # Beregner endings-skår
        mutate(str_change = post - pre) %>%
        # Lagrer endrings-skår og utvider med sett
...
#| message: FALSE
#| include: FALSE
#| fig-cap: " Figur 2 - endring i muskelstyrke ved Legpress"
...
Den parrede t-test sammenligner endringen i muskelstyrke  ved Legpress gjennom intervensjonen  (pre-post) sett i forhold til de to gruppene (multiple- og single sett).
Beregningen viser at t = 2.1366, df = 29, p-value = 0.0412 og endringen i muskelstyrken i forhold til de to gruppene ved legpress da er signifikant.
p \<0.05.
...
#| message: FALSE
#| include: FALSE
#| Beregner endring av styrke mellom gruppene 
...
@mangine_improving_2015 og @pina2019 gjennomførte et studie på eldre kvinner, hvor de trente enten to (G2) eller tre (G3) ganger i uken.
Totalload var altså større for (G3), men det var ingen signifikant forskjell mellom gruppene ift hypertrofi og styrke.
En metaanalyse av @refalo_influence_2021 viser at trening med både høy og lav load ga hypertrofi, mens høy load ga større økning i styrke.
Dette samsvarer til dels med våre funn.
@lopesmachado2022 finner ingen dierke sammenheng mellom intensitet og volum og hypertrofi.
